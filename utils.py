...
def listdir_sm(network_dir):
    ...
    for file_path in temp_sm_file_list:
        ...
        if temp:
            ...
            # if float(temp.group().split('_')[1])<=0.1: #the depth label on the file name is 0.1 for most us sites...
...
def readstm_all(file,var_name,s_time,e_time):
    ...
    df_timeframe = pd.date_range(start="2020-01-01", end="2022-12-31", freq='d').rename('time').to_frame().reset_index(drop=True)
